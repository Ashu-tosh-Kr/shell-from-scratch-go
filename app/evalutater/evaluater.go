...
func (e *Evaluator) Eval(stmt ast.BaseCmd, stdIn io.ReadCloser, stdOut io.WriteCloser, stdErr io.WriteCloser) {
	...
	case ast.PipedCmd:
		...
		go func() {
			defer w.Close()
			e.Eval(stmt.Left, stdIn, w, stdErr)
		}()
		e.Eval(stmt.Right, r, stdOut, stdErr)
	...
	case ast.RedirectCmd:
		...
		e.Eval(stmt.Cmd, stdIn, outW, errW)
	}
}
